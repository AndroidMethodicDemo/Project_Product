...
//		mListView.addHeaderView(placeHolderView);
	...
	public void onActivityCreated(Bundle savedInstanceState) {
		...
		arrayAdapter = new ArrayAdapter<String>(getActivity(), R.layout.list_item, android.R.id.text1, mListItems);
		mListView.setAdapter(arrayAdapter);
		
		((RefreshListView)mListView).setOnRefreshListener(new LoadDataCallback() {
			
			@Override
			public BaseAdapter loadNewData() {
	
				SystemClock.sleep(2000);
				mListItems.add(0, "这是刷新出来的数据");
//				adapter.notifyDataSetChanged();
	...
	public void onScrollStateChanged(AbsListView view, int scrollState) {
		if (mScrollTabHolder != null)
			mScrollTabHolder.onScrollStateChanged(view, scrollState, mPosition);
	}
